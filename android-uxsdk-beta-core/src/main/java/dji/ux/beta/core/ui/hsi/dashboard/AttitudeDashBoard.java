...
import dji.ux.beta.core.widget.hsi.AttitudeDisplayModel;
...
public class AttitudeDashBoard extends ScrollableAttributeDashBoard {
    ...
    private boolean mShowDownwardVisualPerceptionInfo = true;
    ...
    protected void onAttachedToWindow() {
        ...
        // 上视障碍物距离
        // 下视障碍物距离
        mCompositeDisposable.add(widgetModel.getPerceptionFullDistanceProcessor().toFlowable().subscribe(perceptionPushOmnidirectionalRadarStatus -> {
            mUVisionDistance = perceptionPushOmnidirectionalRadarStatus.getUpwardObstacleDistance() * 1f / 1000;
            mDownVisionDistance = perceptionPushOmnidirectionalRadarStatus.getDownwardObstacleDistance() * 1f / 1000;
            updateWidget();
        }));

        //用户设置的上视障碍物避障距离
        mCompositeDisposable.add(widgetModel.getUpAvoidanceDistanceProcessor().toFlowable().subscribe(value -> {
            mUserSetUpBarrierAvoidanceDistance = value;
            updateWidget();
        }));

        //用户设置的上视障碍物告警距离
        mCompositeDisposable.add(widgetModel.getOmniUpRadarDistanceProcessor().toFlowable().subscribe(value -> {
            mUserSetUpBarrierWarnDistance = value;
            updateWidget();
        }));

        //用户设置的下视障碍物避障距离
        mCompositeDisposable.add(widgetModel.getDownAvoidanceDistanceProcessor().toFlowable().subscribe(value -> {
        ...
        mCompositeDisposable.add(widgetModel.getOmniDownRadarDistanceProcessor().toFlowable().subscribe(value -> {
            mUserSetDownBarrierWarnDistance = value;
            updateWidget();
        }));

        //视觉感知避障开关
        mCompositeDisposable.add(widgetModel.getOmniUpwardsAvoidanceEnabledProcessor().toFlowable().subscribe(aBoolean -> {
            mUpwardVisualEnable = aBoolean;
            updateWidget();
        }));

        mCompositeDisposable.add(widgetModel.getVelocityZProcessor().toFlowable().subscribe(speedZ -> {
            mSpeedZ = speedZ;
            updateWidget();
        }));
        ...
        mCompositeDisposable.add(widgetModel.getReturnHomeFlightHeightProcessor().toFlowable().subscribe(integer -> {
        ...
        mCompositeDisposable.add(widgetModel.getLimitMaxFlightHeightInMeterProcessor().toFlowable().subscribe(height -> {
            mLimitHeight = height.intValue();
            updateWidget();
        }));

//        mCompositeDisposable.add(Pilot2Repo.FlightAssistant().LandingProtectionEnabled().observable(this).subscribe(aBoolean -> {
...
//        }));
    ...
    private void drawBarrierInfo(Canvas canvas, int orientation) {
        ...
        float ratio = (float) mAvoidanceMaxHeight / getBarrierValue(mMaxPerceptionDistanceInMeter);
        // 绘制上视障碍物最远感知条
    ...
    private int drawMaxPerceptionIndicator(Canvas canvas, int orientation, float ratio) {
        ...
        if (outOfPerceptionDistanceRange > 0) {
            ...
            canvas.drawRect(RECT, mPaint);
            canvas.translate(0, (int) outOfPerceptionArea * orientation);
    ...
    private int drawSettingPerceptionIndicator(Canvas canvas, int orientation, int restArea, float ratio) {
        ...
        if (upPerceptionArea > 0) {
            ...
            RECT.set(-mAvoidanceIndicatorWidth, top, 0, bottom);
            canvas.drawRect(RECT, mPaint);
            canvas.translate(0, (int) upPerceptionArea * orientation);
        }
        return restArea;
    }
    ...
    private int drawBarrierIndicator(Canvas canvas, int orientation, int restArea, float ratio) {
        ...
        if (upBarrierArea > 0) {
            ...
            RECT.set(-mAvoidanceIndicatorWidth, top, 0, bottom);
            canvas.drawRect(RECT, mPaint);
            canvas.translate(0, (int) upBarrierArea * orientation);
        }
        return restArea;
    }
    ...
    /**
     * https://confluence.djicorp.com/pages/viewpage.action?pageId=178561760
     * @param barrierDistance
     * @return
     */
